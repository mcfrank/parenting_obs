...
d = read.csv("../exp1_lexical_diversityAdd.csv") # with Conversational_Turns, but only has 53 subjects !!
...
e1ld <- e1ld %>% 
  mutate(types = (types / play_duration), 
         ...
         Conversational_Turns = (Conversational_Turns / play_duration))
...
d = read.csv("../exp2_lexical_diversityAdd.csv")
...
#  select(sid, vid_dur)
#e2data <- e2data %>% left_join(e2_dur, by="sid")
...
e2_play_duration <- e2_play_duration %>% 
  ...
  select(sid, play_duration)
e2data <- e2data %>% left_join(e2_play_duration, by="sid")
...
The two coders had a reliability of ICC = 0.79 with 95% confidence interval (CI) = [0.55, 0.91] for rate of parent bids for JA (number of bids per minute); ICC = 0.34 with 89% CI = [-0.11, 0.67] for rate of passive JA episodes (per minute); ICC = 0.66 with 89% CI = [0.32, 0.85] for rate of coordinated JA episodes; ICC = 0.33 with 89% CI = [-0.11, 0.67] for rate (seconds per minute) of passive JA episodes, and ICC = 0.60 with 89% CI = [0.24, 0.82] for rate (seconds per minute) of coordinated JA episodes. 


## Results and Discussion

Parents' child-directed speech during the play sessions was transcribed; child utterances were not considered.
...
For each transcript of child-directed speech, the words were lemmatized using ``spacy2`` [@spacy2], and the word *types* (unique words) and *tokens* (total words) were then tallied and converted to rates (e.g., tokens per minute of play), and the type-token ratio (TTR) was calculated as a measure of lexical diversity. 
...
summary(modTTR) # n_eff ~2100 (is n_eff / n = Effective Sample Size (ESS)?)

e1TTR = get_stan_glmer_reporting_values(modTTR, "conditionexp") # -.14  !!!
p_direction(modTTR, effects="all") # orchestra 78.7%

#ci_hdi <- ci(modTTR, method = "HDI") # [-0.25, -0.01]
#ci_eti <- ci(modTTR, method = "ETI") # [-0.26, -0.01]
...
p_direction(modTTRE, effects="all") # EL 75, RR 86%, parent_ed 83.5

modMTLDE <- stan_glmer(paste("MTLD", e1model_string), data=e1ld)
summary(modMTLDE) #
p_direction(modMTLDE, effects="all") # RR 81.5% genderM 88.9% 
...
p_direction(tokens_modE, effects="all") # RR 71% parent_ed 90%

# predict word types
types_modE <- stan_glmer(paste("types", e1model_string), data=e1ld) 
summary(types_modE) 
p_direction(types_modE, effects="all") # EL 82.5% AA 87% genderM 93.2%, parent_ed 93.5% cond:AA 94.5%
get_stan_glmer_reporting_values(types_modE, 'parent_ed') 
get_stan_glmer_reporting_values(types_modE, 'conditionexp:AA') 

# predict # of conversational turns - COULD REPORT THIS
modCT <- stan_glmer(Conversational_Turns ~ condition * age + (1|video), data=e1ld)
summary(modCT) 
get_stan_glmer_reporting_values(modCT, "conditionexp") # pd = .96 mean=2.52 [-.20,5.28]
p_direction(modCT, effects="all")
# E1 CT: exp early significant (+ effect)
# E2 CT: age nearly significant (+ effect)
```
...
For effects that are at least 95% likely to be non-zero according to the posterior distribution, we report estimated coefficients ($\beta$) as well as 89% Bayesian credible intervals (89% CI)[^4], demarcating the range within which 89% of the posterior falls, meaning that given the observed data, the effect has 89% probability of falling within this range.
There was lower TTR in the Activity Video condition (mean: `r round(subset(e1_lexdiv, condition=='exp')$TTR, 2)`) than in the No Video condition (mean: `r round(subset(e1_lexdiv, condition=='con')$TTR, 2)`, $\beta=$`r e1TTR["conditionexp"]`, 89% CI=[`r e1TTR['89pLB']`, `r e1TTR['89pUB']`]). 
A similar regression instead predicting MTLD also found lower lexical diversity in the Activity Video condition (mean MTLD: `r round(subset(e1_lexdiv, condition=='exp')$MTLD, 2)`) than in the No Video condition (mean: `r round(subset(e1_lexdiv, condition=='con')$MTLD, 2)`, $\beta=$`r e1MTLD["conditionexp"]`, 89% CI=[`r e1MTLD['89pLB']`, `r e1MTLD['89pUB']`]), with no predicted influence of age.
Figure \ref{fig:lexdiv} shows the mean of each lexical diversity measure (TTR and MTLD) by condition.

[^4]: 89% CIs are recommended for Bayesian analyses because unless the effective sample size (ESS) is on the order of 10,000, the 95% credible interval is unstable [@Kruschke2014; @McElreath2018]. Our ESS is approximately 2,100, and thus we go with the tighter, more stable interval.
...
p_direction(e1bids_mod, effects="all")
...
p_direction(e1cja_mod, effects="all") # condition 80%
...
p_direction(e1pja_time_mod, effects="all") # condition 85.7%
...
p_direction(e1cja_time_mod, effects="all") # cond*age 85%
```
...
Parents' bid rate was greater in the Activity Video condition (mean: `r round(subset(e1bids_M, Condition=="Activity Video")$mean,2)`, sd: `r round(subset(e1bids_M, Condition=="Activity Video")$sd,2)`) than in the No Video condition (mean: `r round(subset(e1bids_M, Condition=="No Video")$mean,2)`, sd: `r round(subset(e1bids_M, Condition=="No Video")$sd,2)`, $\beta=`r e1bids["ConditionActivity Video"]`$, 89% CI=[`r e1bids['89pLB']`, `r e1bids['89pUB']`]). 
...
p_direction(e1cja_modE, effects="all") # RR 86% parent_ed 89%

# Episodes of passive joint attention.
e1pja_modE <- stan_lmer(paste("pja", model_stringE), data = e1ja)
summary(e1pja_modE) # condition*RR beta=1.8, RR close
e1pjaRR = get_stan_glmer_reporting_values(e1pja_modE, "RR") # -.34, 89% CI = [-.65,-.02]  REPORT
#e1pja_parent_ed = get_stan_glmer_reporting_values(e1pja_modE, "parent_ed") # -.23 [-.47,.02] pd=.97 REPORT?
e1pjaE = get_stan_glmer_reporting_values(e1pja_modE, "ConditionActivity Video:RR") # .60 [.14,1.05]
p_direction(e1pja_modE, effects="all")
...
p_direction(e1cja_time_mod, effects="all") # condition 85.1%
```
...
Of these exploratory regressions, only the regression predicting the rate of passive JA episodes showed an effect involving these demographic variables: parents scoring higher on the Rules and Respect (RR) subscale had a lower rate of passive JA episodes ($\beta=`r e1pjaRR["ConditionActivity Video:RR"]`$, 89% CI=[`r e1pjaRR['89pLB']`, `r e1pjaRR['89pUB']`]).
However, higher RR parents in the Activity Video condition did not show a decrease in passive JA (interaction of condition and RR:  $\beta=`r e1pjaE["ConditionActivity Video:RR"]`$, 89% CI=[`r e1pjaE['89pLB']`, `r e1pjaE['89pUB']`]).
...
[^5]: Preregistration: [https://osf.io/2bpdf/](https://osf.io/2bpdf). 

## Method


### Participants. 
`r nrow(e2data)` infants (F = `r e2_gender['F']`, M = `r e2_gender['M']`) aged 12-24 months 
...
The two coders had a reliability of ICC = 0.81 with 95% confidence interval (CI) = [0.62,0.91] for number of parent bids for JA; ICC = 0.74 with 89% CI = [0.48, 0.88] for number of passive JA episodes; ICC = 0.80 with 89% CI = [0.61, 0.91] for number of coordinated JA episodes; ICC = 0.72 with 89% CI = [0.44, 0.86] for total duration of passive JA episodes, and ICC = 0.88 with 89% CI = [0.75, 0.94] for total duration of coordinated JA episodes. 
...
summary(e2TTR_m) # conditionexp beta=-.1 89% CI=[-.2,0.0]
e2TTR <- get_stan_glmer_reporting_values(e2TTR_m,"conditionexp") # -.09 [-.15,-.04]
p_direction(e2TTR_m, effects="all") # age 85.6% age:cond 78%
...
summary(e2MTLD_m) # NA
p_direction(e2MTLD_m, effects="all") # cond 77.9% age:cond 84.2%
...
This revealed lower TTR after the Activity Video (mean: `r round(e2_lexdiv[2,]$TTR, 2)`) than after the Science Video (mean: `r round(e2_lexdiv[1,]$TTR, 2)`, $\beta=`r e2TTR["conditionexp"]`$, , 89% CI=[`r e2TTR['89pLB']`, `r e2TTR['89pUB']`]). 
...
e2types <- ggplot(e2typ, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  ...
  xlab("Condition") + ylab("Word Types per Minute") +
  langcog::scale_fill_solarized() + ggthemes::theme_few() +
  theme(legend.position="none") 
...
p_direction(e2bids_m, effects="all") # age 82%
...
Shown in Figure \ref{fig:JA} (left bottom), parents made bids for JA at a greater rate after watching the Activity Video (mean: `r round(subset(e2_bids, Condition=="Activity Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_bids, Condition=="Activity Video")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Activity Video")$ci_upper,2)`]; $\beta=`r e2bids["ConditionActivity Video"]`$, 89% CI = [`r e2bids["89pLB"]`, `r e2bids["89pUB"]`]) than after the Science Video (mean: `r round(subset(e2_bids, Condition=="Science Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_bids, Condition=="Science Video")$ci_lower,2)`, `r round(subset(e2_bids, Condition=="Science Video")$ci_upper,2)`]). 
There were no other effects on parental bids for JA.

The regression predicting rate of JA episodes revealed an effect of condition ($\beta=`r e2tjaCond["ConditionActivity Video"]`$, 89% CI = [`r e2tjaCond["89pLB"]`, `r e2tjaCond["89pUB"]`]), with JA episodes occurring at a greater rate after the Activity Video (mean: `r round(subset(e2_tja, Condition=="Activity Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_tja, Condition=="Activity Video")$ci_lower,2)`, `r round(subset(e2_tja, Condition=="Activity Video")$ci_upper,2)`]) than after the Control Video (mean: `r round(subset(e2_tja, Condition=="Science Video")$mean,2)`, 95% conf. int.: [`r round(subset(e2_tja, Condition=="Science Video")$ci_lower,2)`, `r round(subset(e2_tja, Condition=="Science Video")$ci_upper,2)`]).
Older children also participated in JA episodes at a lesser rate than younger children ($\beta=`r e2tjaAge["age"]`$, 89% CI = [`r e2tjaAge["89pLB"]`, `r e2tjaAge["89pUB"]`]). 
However, this age effect was moderated in the Activity Video condition ($\beta=`r e2tjaAgeCond["age:ConditionActivity Video"]`$, 89% CI = [`r e2tjaAgeCond["89pLB"]`, `r e2tjaAgeCond["89pUB"]`]): shown in Figure \ref{fig:JA} (right bottom), older children did not engage in episodes of JA at a lower rate after an activity video.
...
p_direction(e2cja_m, effects="all") # cond, age:cond
...
p_direction(e2cjat_m, effects="all") # age, cond 84.4%, age:cond 86.2%

e2tjat_m <- stan_glmer(total_lja ~ age * Condition + (1|video), data=e2ja)
summary(e2tjat_m) # anything here? close!
p_direction(e2tjat_m, effects="all") # age 85.3%, cond 89%, age:cond 90.4%
```
...
The regression predicting the rate of coordinated JA episodes found an effect of condition ($\beta=`r e2cjaCond["ConditionActivity Video"]`$, 89% CI = [`r e2cjaCond["89pLB"]`, `r e2cjaCond["89pUB"]`]), with a greater rate of coordinated JA episodes occurring after the Activity Video (mean: `r round(subset(e2_cja, Condition=="Activity Video")$mean,2)`, 95% CI: [`r round(subset(e2_cja, Condition=="Activity Video")$ci_lower,2)`, `r round(subset(e2_cja, Condition=="Activity Video")$ci_upper,2)`]) than after the Control Video (mean: `r round(subset(e2_cja, Condition=="Science Video")$mean,2)`, 95% CI: [`r round(subset(e2_cja, Condition=="Science Video")$ci_lower,2)`, `r round(subset(e2_cja, Condition=="Science Video")$ci_upper,2)`]).
There was an interaction of age and condition ($\beta=`r e2cjaAgeCond["age:ConditionActivity Video"]`$, 89% CI = [`r e2cjaAgeCond["89pLB"]`, `r e2cjaAgeCond["89pUB"]`]), shown in Figure \ref{fig:e2ja-coord}, revealing that after an Activity Video older children participated in coordinated JA episodes at a greater rate than children in the Control Video condition.
The regression predicting the time spent in coordinated JA episodes found no notable effects.

Older children both engaged in episodes of passive JA at a greater rate with their caregiver ($\beta=`r e2pja["age"]`$, 89% CI = [`r e2pja["89pLB"]`, `r e2pja["89pUB"]`]), and spent more time in passive JA with their caregiver ($\beta=`r e2pjat["age"]`$, 89% CI = [`r e2pjat["89pLB"]`, `r e2pjat["89pUB"]`]).
...
The words that were strongly indicative of being from the Activity Video condition include "big", "little", "give", "small", "cow", "yellow", "take", and "put", while words that were most characteristic of the Science Video condition include "beep", "like", "neigh", "uhoh", "say", "for", "does", and "did". 
