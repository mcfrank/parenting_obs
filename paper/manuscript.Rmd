...
diagnostic_posterior(modTTR) # 2500-4100
...
summary(types_modE) 
p_direction(types_modE, effects="all") # cond 82.5% EL 82.5% AA 87% genderM 93.2%, parent_ed 93.5% cond:AA 94.5%
...
[^4]: 89% CIs are recommended for Bayesian analyses because unless the effective sample size (ESS) is on the order of 10,000, the 95% credible interval is unstable [@Kruschke2014; @McElreath2018]. Our ESS ranges from 2,000-5,000, and thus we go with the tighter, more stable interval.
...
Of these exploratory regressions, only the regression predicting the rate of passive JA episodes showed an effect involving these demographic variables: parents scoring higher on the Rules and Respect (RR) subscale had a lower rate of passive JA episodes ($\beta=`r e1pjaRR["ConditionActivity Video:RR"]`$, `r print_CI(e1pjaRR)`).
However, higher RR parents in the Activity Video condition did not show a decrease in passive JA (interaction of condition and RR:  $\beta=`r e1pjaE["ConditionActivity Video:RR"]`$, `r print_CI(e1pjaE)`).
...
We first report preregistered regressions[^6] predicting TTR and number of tokens per minute of play, as well as an exploratory regression predicting MTLD per minute.
...
# NOT INCLUDED
e2typ <- e2ld %>% group_by(Condition) %>%
  tidyboot_mean(types) 

e2types <- ggplot(e2typ, aes(x = Condition, y = mean, fill = Condition)) + geom_bar(stat="identity") + 
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), position = position_dodge(width = .9)) +
  geom_point(data=e2ld, aes(x=Condition, y=types), alpha=.2, position = position_jitterdodge()) +
