...
However, higher RR parents who saw an Activity Video did not show this decrease in passive JA (interaction of condition and RR:  $\beta=`r e1pjaE["ConditionActivity Video:RR"]`$, `r print_CI(e1pjaE)`). 
This regression also found that more educated parents showed a lower rate of passive JA episodes ($\beta=`r e1pja_parent_ed['parent_ed']`$, `r print_CI(e1pja_parent_ed)`).
Finally, parents made bids for JA at a higher rate with male children than with female children (male mean: `r round(e1bidsE_M[2,]$mean,2)` bids/min, female mean: `r round(e1bidsE_M[1,]$mean,2)`, $\beta=`r e1bidsE["genderM"]`$ `r print_CI(e1bidsE)`).
