...
abstract: |
  Early parenting practices help shape children’s future outcomes. In particular, high-quality early interactions can facilitate language learning and school performance. Phone-based parenting applications can deliver low-cost interventions on parenting style. In two studies (N=142; 79 female; ages 6-24 months; 74 white, 31 Asian, 31 biracial, 3 black, 12 other; 31 Hispanic), we investigated effects of short videos on parent behavior. Immediately after parents watched videos depicting age-appropriate parent-child activities, parents spoke more (d=0.83) and made more bids for joint attention (d=1.03). These results suggest that activity videos can lead to near-term changes in parent engagement, providing support for the use of such videos as a part of broader app-based parenting interventions.
...
In this paper, rather than conducting a large-scale intervention, we investigate the ways that parents change their local (i.e. near-term) behavior with children immediately after seeing a short video instructing them on an activity, investigating one of the building blocks of many larger interventions.
...
Both within and outside the realm of academic interventions, messages to parents of young children often seek to provide knowledge about some aspect of development (e.g., early language; @earlylangint), often in tandem with a suggestion regarding activities (e.g., book reading; for a review see @Reese2010review).
...
Episodes of joint attention are frequent during guided play, when parents set goals (e.g., building a tower) and scaffold their child's activities [@Wood1976; @Weisberg2013].
...
For example, focusing parents on a specific activity could cause parents to over-focus on achieving the superficial goals of the activity while neglecting the broader goals of fostering more -- and more responsive -- interaction. 
...
Attempting to reproduce the surface details of a video-guided activity could in turn result in less high-quality talk, with less responsiveness to their child's play, a target of parent responsiveness interventions [e.g., @Landry2008].
...
Our current studies were designed to assess how parents change their near-term interactions with young children on the basis of these videos. 
...
In Experiment 1, we invited parents of 6- to 24-month-old infants visiting [a children's museum in XXXX] to complete video-guided activities from a commercial parenting app that delivers digital parenting advice in the form of short videos. <!-- the Children's Discovery Museum in San Jose -->
...
### Participants
`r nrow(e1ld)` infants (F = `r e1_gender['F']`, M = `r e1_gender['M']`) aged 6-24 months (19 6-11.9 month-olds: 9 in the control condition, 10 in Activity Video; 20 12-17.9 month-olds: 11 control, 9 Activity Video; 21 18-24 month-olds: 10 control, 11 Activity Video) and their parents participated in a museum in [XXXX]. <!-- San Jose, CA --> 
...
Without their knowledge, participants were randomly assigned to either the *Activity Video* condition or the *No Video* condition. 
...
The EPAQ measures parents of young children's attitudes about parenting and child development along three dimensions: rules and respect, early learning, and affection and attachment ([see SI](https://osf.io/2bpdf/?view_only=69c0c6ca5d2649438e8642a57671ba79)). <!-- https://osf.io/2bpdf/?view_only=69c0c6ca5d2649438e8642a57671ba79 -->
...
The toys were placed on a large foam play mat, and parents were instructed to sit on the mat with their child and re-create the activity they had viewed for a period of approximately three minutes.^[Based on piloting, we estimated these activities would would only require three minutes to complete.]
...
[^3]: [XXXX]
<!-- https://osf.io/6k9m8/](https://osf.io/6k9m8/?view_only=d4b230deba2a4f91bdb2aba29088d982) -->
...
Words that were strongly indicative of being from the Activity Video condition include "give", "big", "small", "ribbit", "thank", "have", and "bus", while words that were most characteristic of the No Video condition include "shake", "ready", "oh", "on", "going", "see", "let", and the child's name. 
...
[^5]: Preregistration: [XXXX] <!-- [https://osf.io/6k9m8/](https://osf.io/6k9m8/?view_only=d4b230deba2a4f91bdb2aba29088d982). -->
...
ttr_means <- e2ld %>% group_by(condition) %>%
  summarise(mean = mean(TTR), sd = sd(TTR))
cohens_d(ttr_means) # -1.28

# overall effect sizes for abstract:
e1e2_tok <- e1ld %>% select(sid, condition, types, tokens, TTR, MTLD) %>%
  bind_rows(e2ld %>% select(sid, condition, types, tokens, TTR, MTLD)) %>% 
  group_by(condition) %>%
  summarise(mean = mean(tokens), sd = sd(tokens))
cohens_d(e1e2_tok) # d=0.83 (E1: 0.94, E2: .77)

e1e2_ja <- e1ja %>% mutate(condition = factor(ifelse(Condition=="No Video", "con", "exp"))) %>%
  select(SID, condition, bids, pja, cja) %>%
  bind_rows(e2ja %>% select(SID, condition, bids, pja, cja)) %>% 
  group_by(condition) %>%
  summarise(mean = mean(bids), sd = sd(bids))
cohens_d(e1e2_ja) # d=1.03
# After watching the videos, parents spoke more (d=0.83) and made more bids for joint attention (d=1.03).
```
...
XXXX
<!-- This work was supported by both an unrestricted gift and a research contract from Kinedu, Inc. to the Language and Cognition Lab.  -->
