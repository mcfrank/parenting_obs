...
53 infants (F = 38, M = 15) aged 6-24 months (20 6-11.9 month-olds, 20 12-17.9 month-olds, and 20 18-24 month-olds) and their parents participated in a museum in northern California.
...
Although Experiment 1 was not preregistered, for consistency the transcripts and hand-coded joint attention data were analyzed according to our preregistration for Experiment 2\footnote{Preregistration: \url{https://osf.io/2bpdf/}{]}}, with any deviations or exploratory analyses noted.
...
For each transcript, the words were lemmatized using \texttt{spacy2} (Honnibal, 2017), and the word \emph{types} (unique words) and \emph{tokens} (total words) were then tallied and converted to rates (e.g., tokens per minute of play), and the type-token ratio (TTR) was calculated as a measure of lexical diversity.
...
There was lower TTR in the Activity Video condition (mean: 0.33) than in the No Video condition (mean: 0.43, \(\beta=\)-0.11, 95\% CI={[}-0.16,-0.05{]}).
A similar regression instead predicting MTLD also found lower lexical diversity in the Activity Video condition (mean MTLD: 18.55) than in the No Video condition (mean: 27.09, \(\beta=\)-8.48, 95\% CI={[}-15.48,-1.45{]}), with no predicted influence of age.
Figure \ref{fig:lexdiv} shows the mean of each lexical diversity measure (TTR and MTLD) by condition.

We also conducted similar regressions predicting the rate of word tokens and types per minute of play, finding an effect of condition on the number of word tokens (\(\beta=\) 19.45, 95\% CI={[}8.93,30.24{]}), with parents using more words in the Activity Video condition (mean: 60, bootstrapped 95\% confidence interval (conf. int.): {[}193,259{]}) than in the No Video condition (mean: 41, 95\% conf. int.: {[}139,194{]}).
...
Parents' bid rate was greater in the Activity Video condition (mean: 1.83, sd: 0.79) than in the No Video condition (mean: 1.10, sd: 0.83, \(\beta=0.71\), 95\% CI={[}0.15,1.27{]}).
...
Of these exploratory regressions, only the regression predicting the rate of passive JA episodes showed an effect involving these demographic variables: parents scoring higher on the Rules and Respect (RR) subscale a lower rate of passive JA episodes (\(\beta=NA\), 95\% CI={[}-0.65,-0.02{]}).
However, higher RR parents in the Activity Video condition did not show a decrease in passive JA (interaction of condition and RR: \(\beta=0.60\), 95\% CI={[}0.11,1.04{]}).
...
This revealed lower TTR after the Activity Video (mean: 0.38) than after the Science Video (mean: 0.48, \(\beta=-0.09\), , 95\% CI={[}-0.15,-0.04{]}).
