...
play_duration <- play_duration %>% 
  mutate(play_duration = play_stop_sec - play_start_sec)
...
d.long[which(d.long$phase_length<0),] # 052317_03	7	-93378	0	0	1 (only a bid, so duration doesn't matter)
...
bids_rel = get_ICC2(d_rel[,c("bids","bids_rel")]) # .38 [.02, .66]

pja_rel = get_ICC2(d_rel[,c("pja","pja_rel")]) # eps pass JA .06 [-.19, .36]

cja_rel = get_ICC2(d_rel[,c("cja","cja_rel")]) # eps coord JA .59 [.27, .79]

l_pja_rel = get_ICC2(d_rel[,c("l_pja","l_pja_rel")]) # pass JA length .15 [-.13, .46]

l_cja_rel = get_ICC2(d_rel[,c("l_cja","l_cja_rel")]) # coord JA length .37 [-.06, .68]
...
ggplot(ms_bids, aes(x = Condition, y = mean, fill = Condition)) + 
  ...
  xlab("Condition") + 
  ylab("Bids for Joint Attention per Min.") +
...
ggplot(ms_tja, aes(x = Condition, y = mean, fill = Condition)) + 
  ...
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9))+
  xlab("Condition") + 
  ylab("Episodes of Joint Attention per Min.") +
  langcog::scale_colour_solarized()  +
  ggthemes::theme_few() + 
...
ggplot(ms_lpja, aes(x = Condition, y = mean, fill = Condition)) + 
  ...
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9))+
  xlab("Condition") + 
  ylab("Seconds of Passive JA per Min.")  +
  langcog::scale_colour_solarized()  +
  ggthemes::theme_few() 
```
...
ggplot(ms_lcja, aes(x = Condition, y = mean, fill = Condition)) + 
  ...
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), 
             position = position_dodge(width = .9))+
  xlab("Condition") + 
  ylab("Seconds of Coordinated JA per Min.")  +
  langcog::scale_colour_solarized()  +
  ggthemes::theme_few() 
```
